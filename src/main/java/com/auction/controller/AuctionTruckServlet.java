...
@WebServlet(name = "AuctionTruckServlet", value = "/auctiontruck")
public class AuctionTruckServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        // Make sure user is logged in
        if (request.getSession().getAttribute("user") != null) {
            getServletContext().getRequestDispatcher("/WEB-INF/views/auctiontruck.jsp").forward(request, response);
        } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (auctionDao.vehicleIsListed(vin)) {
            ...
            v.setBed_size(Integer.parseInt(request.getParameter("bedsize")));
            v.setCar(false);
            v.setTruck(true);
